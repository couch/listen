...
- Quick-return resume (mobile): backgrounding the browser lets the OS pause playback; returning to the foreground within 30 s resumes the same track automatically (a longer absence stays paused, so audio never resumes unexpectedly after a real departure)
...
- Playlist footer: created and edited dates below the track list, each with an outline glyph tied to its line (sprout = created, cycle arrows = edited, map pin = the distance line) in the number column, text under the track titles, aligned to the track grid; no track count — the track list is already numbered; scroll to reveal

...
- Wiring in `main.js`: `tickDrift` writes `--bg`/theme-color/`setVizBgColor(hex)` at 10 Hz (`DRIFT_WRITE_MS` throttle via `updateDue` — invisible on a 45 s ramp, avoids per-frame style invalidation); `enableMotionListeners()` feeds `deviceorientation` → `setVizOrientation(beta, gamma)`; on iOS the grant comes solely from the ⊙ click via the `onUserOpen` opt (`requestVizOrientation` — invoked synchronously in the click handler, the gesture context iOS requires; auto-reopen never calls it; orientation only). There is no longer a π button: geolocation is the footer's tap-to-reveal distance line (`initLocationLine`/`requestViewerGeo` in main.js — a tap-gated request with a Permissions-API silent upgrade for prior grantors; `viewerCoords` cached for the session, re-resolved per tape on switch), and Android attaches motion listeners at startup without a prompt. `initVisualizer` opts also carry `isPlaying()` (gates auto-reopen) and `onOpenChange(open)` (main.js stops the ambient orbs / pride canvas while the opaque overlay hides them, resumes on close if playing; the PLAYING branch gates `startAmbient`/`startPrideCanvas` on `!isVisualizerOpen()` for the same reason).
...
- **Controller contract** (`createXSource({ onReady, onState, onError }) → SourcePlayer`): `isReady()`, `load(track, { startSeconds, cue })` (cue = load-without-play for saved-position restore; `startSeconds` without cue = start playback from that offset, the resume-row path; **must be callable synchronously in a gesture — never await before play**), `play/pause/stop`, `seekTo(s)`, `getCurrentTime/getDuration` (0 when unknown), `getState()`. Errors normalize to `'unplayable'` (fatal for this track → main.js skips via `next()`, unless `lastLoadWasCue`) or `'blocked'` (autoplay denied → paused UI, never skip).
- **Wiring in main.js — the global player bar**: the playing tape (`playingTape`, the tape object owning the loaded track) is decoupled from the displayed `tape`; `currentIndex` indexes `playingTape.tracks`, and the linked predicate (`isLinked()` — pure object identity, every path that makes a tape playing assigns the same object) gates all row-coupled DOM work (row classes, `.track-progress`, marquees, per-track pride `--bg`). `load(i, { startSeconds, cue, silent, from })` loads `from.tracks[i]` (default the displayed tape; `next`/prev/retry pass `playingTape`) and sets `playingTape = from`. `warmControllers()` instantiates a controller per family present in the displayed tape, at startup and in `applyTape` (gesture-context rule). `load()` checks `isReady()` (not-ready → `pendingLoad`, which carries its tape + resume offset, flushed by `onSourceReady`), then hands off: sets `activeSourceId` *before* stopping the outgoing controller — the **stale-event guard** (`onState`/`onError` filtered on `sid === activeSourceId`) is what keeps a stopped controller's late PAUSED from stomping the incoming track's UI on mixed tapes. `resetPlaybackUI` nulls `active`/`activeSourceId`/`playingTape`. Saved positions key by `playingTape.id` (`getSavedPosition()` reads the *displayed* tape's slot — startup restore, idle auto-cue, resume chips). `maybeRestoreSavedPosition()` runs once (guarded by `savedRestoreDone`, set by any explicit `load`) from whichever `onSourceReady` arrives with the saved track's source ready. A tape switch (`applyTape`) routes through the pure `tapeSwitchAction(occupied, sameId, trackMatches)` (utils.js): an occupied bar (playing, or paused after a PLAYING — the `started` flag; an untouched startup cue is idle) survives the switch — `'detach'` keeps playback and renders the incoming tape's resume chip (`renderResumeIndicator`), `'relink'` (back to the playing tape, `sameTrack` from ids.js still matching at `currentIndex`) adopts the fresh object and re-couples the rows (`relinkRows`), `'reset'` is the pre-global behavior (stop, rebuild, auto-cue). While unlinked, the now-playing block shows a source-tape chip (`#np-tape`, `updateNowPlayingChip`) and tapping it switches back. The buffering watchdog re-checks `active?.getState()`.

### youtube (`src/sources/youtube.js`)

Extraction of the old inline code: IFrame API script load at construction, `YT.Player` on the `#yt-player` div, pure `mapYouTubeState` (−1/0/1/2/3/5 → STATE) and `mapYouTubeError` (every error code is fatal → `'unplayable'`, matching pre-seam behavior). Load dispatch is the pure `loadCommand`: `cue` → `cueVideoById({ videoId, startSeconds })`, play-with-offset → `loadVideoById({ videoId, startSeconds })`, plain play → `loadVideoById(id)` (bare-id call shape preserved).
...
- Cue path: `src` set, seek applied on `loadedmetadata`, then CUED (paints the restored scrubber like YT's cue). Play-with-offset (`startSeconds` without `cue`): `play()` fires synchronously (gesture rule), the seek lands on `loadedmetadata` *without* emitting CUED (it would flip the UI to paused mid-start) — a slow host may audibly play a beat from 0:00 first.
